...
function SwitchTheme() {
    if (getComputedStyle(docEle).getPropertyValue("--darker-color-noa") == "rgb(32, 32, 32)") SetLightTheme();
    else SetDarkTheme();
}
// Set Dark theme
function SetDarkTheme() {
    ...
    for (const code of document.querySelectorAll("code"))code.dispatchEvent(new Event("code_RefreshSH"));
}
// Set Light theme
...
function LoadLastTheme() {
    if (localStorage.getItem("CurrentTheme") == "Light") SetLightTheme();
    else SetDarkTheme();
    if (localStorage.getItem("Mica") == "false") DisableMica();
    else EnableMica();
}
function InitializeFluent() {
    ...
    setTimeout(()=>{
        ...
        document.addEventListener("click", function(e) {
            for (const exp of expanders)if (exp.classList.contains("press")) exp.classList.remove("press");
            for (const item of menu_item_expanders)if (item.classList.contains("press")) item.classList.remove("press");
            for (const item1 of menu_items_select)if (item1.classList.contains("press")) item1.classList.remove("press");
        });
        //#endregion
        //#region Init sliders
        ...
        if (document.getElementById("fluent-context-menu-standalone") != null) {
            ...
            document.querySelector("body").addEventListener("contextmenu", (e)=>{
                e.preventDefault();
                const { left: scopeOffsetX , top: scopeOffsetY ,  } = document.querySelector("body").getBoundingClientRect();
                ...
                // ? normalzie on X
                if (outOfBoundsOnX) normalizedX = scopeOffsetX + document.querySelector("body").clientWidth - contextMenuStandalone.clientWidth;
                // ? normalize on Y
                if (outOfBoundsOnY) normalizedY = scopeOffsetY + document.querySelector("body").clientHeight - contextMenuStandalone.clientHeight;
            ...
            document.addEventListener("click", (e)=>{
                if (e.target.offsetParent != contextMenuStandalone) contextMenuStandalone.classList.remove("visible");
            });
        }
        ...
        if (document.getElementById("fluent-context-menu-standalone-text") != null) {
            ...
            for (const context_menu of input_text)context_menu.addEventListener("contextmenu", (e)=>{
                e.preventDefault();
                const { left: scopeOffsetX , top: scopeOffsetY ,  } = document.querySelector("body").getBoundingClientRect();
                const scopeX = e.pageX - scopeOffsetX;
                const scopeY = e.pageY - scopeOffsetY;
                ...
                let normalizedY = e.pageY;
                // ? normalzie on X
                if (outOfBoundsOnX) normalizedX = scopeOffsetX + document.querySelector("body").clientWidth - contextMenuStandaloneText.clientWidth;
                // ? normalize on Y
                if (outOfBoundsOnY) normalizedY = scopeOffsetY + document.querySelector("body").clientHeight - contextMenuStandaloneText.clientHeight;
            ...
            // Close custom context menu
            document.addEventListener("click", (e)=>{
                if (e.target.offsetParent != contextMenuStandaloneText) contextMenuStandaloneText.classList.remove("visible");
            });
        }
        //#endregion
        Initialized = true;
...
function InitResponsivity() {
    ...
    if (media.matches) responsiveTitleContainer.append(mainMenu);
    else {
...
function InitExpanders() {
    ...
    const expanders = document.querySelectorAll(".fluent-expander-header");
    if (expanders.length != 0) for (const expander of expanders){
        ...
        expander.addEventListener("click", (e)=>{
            ...
            if (expander_body.classList.contains("expanded")) {
                ...
                setTimeout(function() {
                    ...
                    expander.style.cssText = "border-radius: 4px 4px 4px 4px;-webkit-border-radius: 4px 4px 4px 4px;-moz-border-radius: 4px 4px 4px 4px;-ms-border-radius: 4px 4px 4px 4px;-o-border-radius: 4px 4px 4px 4px;";
                }, 280);
            } else {
                expander_body.classList.add("expanded");
                document.documentElement.style.setProperty("--expander-expand-height", expander_body.scrollHeight + "px");
                expander_body.style.animation = "fluent-expander-expand 0.15s ease-in";
                expander.style.cssText = "border-radius: 4px 4px 0px 0px;-webkit-border-radius: 4px 4px 0px 0px;-moz-border-radius: 4px 4px 0px 0px;-ms-border-radius: 4px 4px 0px 0px;-o-border-radius: 4px 4px 0px 0px;";
...
function InitSelectableMenuItems() {
    ...
    if (menus.length != 0) for (const menu of menus){
        const menu_items = menu.querySelectorAll(".fluent-menu-item-select");
        if (menu_items.length != 0) for (const menu_item_select of menu_items){
                ...
                if (!menu_item_select.hasAttribute("selected")) {
                    ...
                    for (const menu_item_select_old of menu_items){
                        if (menu_item_select_old.hasAttribute("selected")) {
                            ...
                            if (newItemIndex > oldItemIndex) menu_item_select_old.firstChild.style.animation = "fluent-menu-item-select-down 0.3s ease-in";
                            else menu_item_select_old.firstChild.style.animation = "fluent-menu-item-select-up 0.3s ease-in";
                            setTimeout(function() {
                    ...
                    if (oldExists) {
                        ...
                        setTimeout(function() {
                            ...
                            // Animate accent colored div movement
                            if (newItemIndex > oldItemIndex) active_element.style.animation = "fluent-menu-item-select-up-reverse 0.3s ease-out";
                            else active_element.style.animation = "fluent-menu-item-select-down-reverse 0.3s ease-out";
                        }, 280);
                    } else {
                        var active_element = document.createElement("div");
                        active_element.classList.add("fluent-menu-item-select-selected");
...
function InitFluentElements() {
    ...
    for (const button of buttons){
        // Set styles
        if (button.hasAttribute("accent")) button.outerHTML = '<button class="fluent-button-accent"' + AttributesToString(button) + ">" + button.innerHTML + "</button>";
        else if (button.hasAttribute("hyperlink")) button.outerHTML = '<button class="fluent-button-hyperlink"' + AttributesToString(button) + ">" + button.innerHTML + "</button>";
        // Get URL attribute
        if (button.hasAttribute("url")) button.addEventListener("click", ()=>{
            window.open(button.getAttribute("url"), "_self");
        });
        else if (button.hasAttribute("urlnew")) button.addEventListener("click", ()=>{
    ...
    for (const link of links){
        if (link.hasAttribute("accent")) link.outerHTML = "<a " + AttributesToString(link) + ' class="fluent-button-accent">' + link.innerHTML + "</a>";
        else if (link.hasAttribute("hyperlink")) link.outerHTML = "<a " + AttributesToString(link) + ' class="fluent-button-hyperlink">' + link.innerHTML + "</a>";
    }
    // Input text
    const inputs_text = document.querySelectorAll('input[type="text"]');
    for (const input_text of inputs_text)input_text.outerHTML = '<div class="fluent-textbox-container"><input ' + AttributesToString(input_text) + " >" + '<button class="fluent-clear-text-input-button">x</button>' + "</div>";
    const inputs_text_new = document.querySelectorAll('input[type="text"]');
    for (const input_text1 of inputs_text_new){
        if (input_text1.hasAttribute("clear_btn")) {
            input_text1.parentElement.lastElementChild.style.opacity = "1";
            input_text1.parentElement.lastElementChild.style.display = "inline";
        }
        input_text1.parentElement.lastElementChild.addEventListener("click", ()=>{
            input_text1.value = "";
            setTimeout(()=>input_text1.focus());
        });
    }
    // Input password
    const inputs_password = document.querySelectorAll('input[type="password"]');
    for (const input_password of inputs_password)input_password.outerHTML = '<div class="fluent-textbox-container"><input ' + AttributesToString(input_password) + " >" + '<a class="fluent-show-password-input" style="opacity: 0; display: none;"><svg style="margin-top: 0.425em;" width="20" height="20" viewBox="0 0 20 20"xmlns="http://www.w3.org/2000/svg"><path class="fluent-svg-path" d="M3.25909 11.6021C3.94254 8.32689 6.79437 6 10 6C13.2057 6 16.0574 8.32688 16.7409 11.6021C16.7974 11.8725 17.0622 12.0459 17.3325 11.9895C17.6029 11.933 17.7763 11.6682 17.7199 11.3979C16.9425 7.67312 13.6934 5 10 5C6.3066 5 3.05742 7.67311 2.28017 11.3979C2.22377 11.6682 2.39718 11.933 2.6675 11.9895C2.93782 12.0459 3.20268 11.8725 3.25909 11.6021ZM10 8C8.067 8 6.5 9.567 6.5 11.5C6.5 13.433 8.067 15 10 15C11.933 15 13.5 13.433 13.5 11.5C13.5 9.567 11.933 8 10 8ZM7.5 11.5C7.5 10.1193 8.61929 9 10 9C11.3807 9 12.5 10.1193 12.5 11.5C12.5 12.8807 11.3807 14 10 14C8.61929 14 7.5 12.8807 7.5 11.5Z" fill="#212121"/></svg></a>' + "</div>";
    const inputs_password_new = document.querySelectorAll('input[type="password"]');
    for (const input_password1 of inputs_password_new){
        const button1 = input_password1.parentElement.lastElementChild;
        if (input_password1.hasAttribute("showpass_btn")) {
            button1.style.opacity = "1";
            button1.style.display = "inline";
        }
        if (input_password1.hasAttribute("showpass")) {
            input_password1.setAttribute("type", "text");
            button1.innerHTML = '<svg style="margin-top: 0.425em;" width="20" height="20" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path class="fluent-svg-path" d="M3.25909 11.6021C3.94254 8.32689 6.79437 6 10 6C13.2057 6 16.0574 8.32688 16.7409 11.6021C16.7974 11.8725 17.0622 12.0459 17.3325 11.9895C17.6029 11.933 17.7763 11.6682 17.7199 11.3979C16.9425 7.67312 13.6934 5 10 5C6.3066 5 3.05742 7.67311 2.28017 11.3979C2.22377 11.6682 2.39718 11.933 2.6675 11.9895C2.93782 12.0459 3.20268 11.8725 3.25909 11.6021ZM9.98953 8C11.9225 8 13.4895 9.567 13.4895 11.5C13.4895 13.433 11.9225 15 9.98953 15C8.05653 15 6.48953 13.433 6.48953 11.5C6.48953 9.567 8.05653 8 9.98953 8Z" fill="#212121"/></svg>';
        } else {
            input_password1.setAttribute("type", "password");
        ...
        button1.addEventListener("click", ()=>{
            if (input_password1.getAttribute("type") == "password" && !input_password1.hasAttribute("showpass")) {
                input_password1.setAttribute("type", "text");
                input_password1.setAttribute("showpass", "");
                button1.innerHTML = '<svg style="margin-top: 0.425em;" width="20" height="20" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path class="fluent-svg-path" d="M3.25909 11.6021C3.94254 8.32689 6.79437 6 10 6C13.2057 6 16.0574 8.32688 16.7409 11.6021C16.7974 11.8725 17.0622 12.0459 17.3325 11.9895C17.6029 11.933 17.7763 11.6682 17.7199 11.3979C16.9425 7.67312 13.6934 5 10 5C6.3066 5 3.05742 7.67311 2.28017 11.3979C2.22377 11.6682 2.39718 11.933 2.6675 11.9895C2.93782 12.0459 3.20268 11.8725 3.25909 11.6021ZM9.98953 8C11.9225 8 13.4895 9.567 13.4895 11.5C13.4895 13.433 11.9225 15 9.98953 15C8.05653 15 6.48953 13.433 6.48953 11.5C6.48953 9.567 8.05653 8 9.98953 8Z" fill="#212121"/></svg>';
            } else {
                input_password1.setAttribute("type", "password");
                input_password1.removeAttribute("showpass", "");
                button1.innerHTML = '<svg style="margin-top: 0.425em;" width="20" height="20" viewBox="0 0 20 20"xmlns="http://www.w3.org/2000/svg"><path class="fluent-svg-path" d="M3.25909 11.6021C3.94254 8.32689 6.79437 6 10 6C13.2057 6 16.0574 8.32688 16.7409 11.6021C16.7974 11.8725 17.0622 12.0459 17.3325 11.9895C17.6029 11.933 17.7763 11.6682 17.7199 11.3979C16.9425 7.67312 13.6934 5 10 5C6.3066 5 3.05742 7.67311 2.28017 11.3979C2.22377 11.6682 2.39718 11.933 2.6675 11.9895C2.93782 12.0459 3.20268 11.8725 3.25909 11.6021ZM10 8C8.067 8 6.5 9.567 6.5 11.5C6.5 13.433 8.067 15 10 15C11.933 15 13.5 13.433 13.5 11.5C13.5 9.567 11.933 8 10 8ZM7.5 11.5C7.5 10.1193 8.61929 9 10 9C11.3807 9 12.5 10.1193 12.5 11.5C12.5 12.8807 11.3807 14 10 14C8.61929 14 7.5 12.8807 7.5 11.5Z" fill="#212121"/></svg>';
            }
        });
    }
    ...
    for (const menu_item of menu_items)if (menu_item.hasAttribute("icon")) {
    ...
    for (const context_menu of context_menus)context_menu.addEventListener("click", (e)=>{
        e.preventDefault();
    });
    // Initialize context menus
    for (const context_menu1 of context_menus){
        const menu_child = context_menu1.querySelector(".fluent-context-menu ul");
        context_menu1.addEventListener("mouseenter", function(e) {
        ...
        context_menu1.addEventListener("mouseleave", function(e) {
    ...
    for (const label of labels){
        if (label.hasAttribute("consolas")) label.style.fontFamily = "Consolas, monaco, monospace";
    ...
    for (const icon1 of icons){
        var iconSVG = document.createElement("img");
        iconSVG.classList.add("fluent-icon");
        iconSVG.setAttribute("src", `https://cdn.spej.eu/fwf/icons/${icon1.getAttribute("icon")}.svg`);
        icon1.outerHTML = iconSVG.outerHTML;
    }
}
...
function ShowFlyout(element, string, delay) {
    ...
    flyout.animate([
        // keyframes
        {
            top: flyout.offsetTop + 5 + "px",
            opacity: "0"
        },
        {
            top: flyout.offsetTop,
            opacity: "1"
        }
    ], {
        // timing options
...
function ShowContentDialog(id) {
    ...
    if (contentDialog != null) {
        ...
        document.body.style.overflow = "hidden";
        setTimeout(()=>{
            contentDialog.parentElement.style.animation = "none";
        }, 105);
    }
}
// Hide content dialog
function HideContentDialog(id) {
    ...
    if (contentDialog != null) {
        ...
        setTimeout(()=>{
            contentDialog.parentElement.style.display = "none";
            document.body.style.overflow = "auto";
        }, 85);
    }
}
...
function CodeSyntaxHigh(elmnt, mode) {
    ...
    /* HTML */ var tagcolor = getComputedStyle(docEle).getPropertyValue("--HTML-tagcolor");
    ...
    /* CSS */ var cssselectorcolor = getComputedStyle(docEle).getPropertyValue("--CSS-selectorcolor");
    ...
    /* JS */ var jscolor = getComputedStyle(docEle).getPropertyValue("--JS-color");
    ...
    function tagMode(txt) {
        var rest = txt, done = "", startpos, endpos, result1;
        ...
        result1 = done + rest;
        result1 = "<span style=color:" + tagcolor + ">&lt;</span>" + result1.substring(4);
        if (result1.substr(result1.length - 4, 4) == "&gt;") result1 = result1.substring(0, result1.length - 4) + "<span style=color:" + tagcolor + ">&gt;</span>";
        return "<span style=color:" + tagnamecolor + ">" + result1 + "</span>";
    }
    ...
    function cssMode(txt) {
        var rest = txt, done = "", s, e, comment, i, midz, c, cc1;
        ...
        rest = rest.replace(/}/g, "<span style=color:" + cssdelimitercolor + ">}</span>");
        for(i = 0; i < comment.arr.length; i++)rest = rest.replace("W3CSSCOMMENTPOS", comment.arr[i]);
    ...
    function jsMode(txt) {
        var rest = txt, done = "", esc = [], i, cc1, tt = "", sfnuttpos, dfnuttpos, compos, comlinepos, keywordpos, numpos, mypos, dotpos, y;
    ...
    function getDotPos(txt, func) {
        ...
        if (s > -1) {
            ...
            for(j = 0; j < x.length; j++){
                cc = x[j];
                for(i = 0; i < arr.length; i++)if (cc.indexOf(arr[i]) > -1) {
                    e = j;
                    return [
                        s + 1,
                        e + s + 1,
...
function getVisibleTextColor(rgba) {
    rgba = rgba.match(/\d+/g);
    if (rgba[0] * 0.299 + rgba[1] * 0.587 + rgba[2] * 0.114 > 186) return "black";
...
//# sourceMappingURL=test.ede7ad7d.js.map
